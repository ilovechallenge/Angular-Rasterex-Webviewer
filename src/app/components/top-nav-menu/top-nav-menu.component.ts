...
import { CollaborationRoomService } from '../collaboration-room/collaboration-room.service';
...
export class TopNavMenuComponent implements OnInit {
  ...
  save() {
    let content = window.prompt("This is prompt");
    console.log("content: ", content)
    this.roomService.saveRunningRoom(this.fileGaleryService.getRoomId()).subscribe(
      (data) => {
          console.log("Response from Save Room", data)
          // window.location.replace('http://viewserver.rasterex.com/collaboration/');
          window.location.reload();
      },
      (error) => {
          console.error("Error from Save Room", error);
      }
    )
  }

  initialize() {
    this.roomService.initialize().subscribe(
      (data) => {
        console.log("Response from Initialize", data)
        // window.location.assign('http://viewserver.rasterex.com/collaboration/');
        window.location.reload();
      },
      (error) => {
          console.error("Error from Initialize", error);
      }
    );
  }
  ...
  ngOnInit(): void {
    this._setOptions();
    this.fileGaleryService.creatorFlag$.subscribe(value => {
      this.isCreator = value
    });
    console.log("isCreator", this.isCreator);
    this.fileGaleryService.roomNameFlag$.subscribe(value => this.roomName = value);
    console.log("roomNameFlag", this.roomName);
  ...
  onActionSelect(): void {
    console.log("This is commentList clicked")
